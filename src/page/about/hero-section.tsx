  ...
  return (
    <section>
      <div className='container'>
        ...
        <ul className={cn('mt-28 flex items-center max-sm:flex-col gap-y-10 gap-x-4')}>
          {heroQualitiesDisplay.map((item) => (
            <li className={cn(locale === 'ar' ? 'flex-row-reverse text-end' : '', 'flex items-start gap-x-6')}>
              <div className={cn('grid shrink-0 place-content-center bg-primary-300 p-2')}>
                <Image
                  alt='icon'
                  height={30}
                  src={item.iconUrl}
